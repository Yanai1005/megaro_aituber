...
const AUTO_COMMENT_PROMPT = 'この画面を見て、ゲームの状況を短く実況してください。'
    ...
    async (text: string) => {
      ...
      if (streamRef.current && videoRef.current) {
        ...
        if (ctx) {
          ...
          homeStore.setState({ modalImage: canvas.toDataURL('image/jpeg', 0.7) })
        }
        ...
        {chatLog
          ...
          .map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                {msg.role === 'user' && <span className="opacity-50 mr-1">あなた:</span>}
      ...
      <div className="absolute top-4 right-4 z-30 flex flex-col items-end gap-2">
        {isStreamActive && (
          <div className="flex items-center gap-1.5 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-xs text-green-400">
            ...
            共有中{autoEnabled && <span className="text-yellow-400 ml-1">・自動実況</span>}
          </div>
        )}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <section className="space-y-2">
            <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider">画面共有</h3>
          ...
          {/* ── 自動実況 ── */}
          <section className="space-y-2">
            <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider">自動実況</h3>
          ...
          {/* ── 質問 ── */}
          <section className="space-y-2">
            <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider">AIに質問</h3>
            <div className="flex gap-2">
              <input
                type="text"
                value={userInput}
          ...
          <section className="space-y-3">
            <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider">AI設定</h3>
            <div>
              <label className="block text-gray-300 text-xs mb-1">サービス</label>
              <select
                value={aiService}
            ...
            <div>
              ...
              />
              <p className="text-gray-600 text-xs mt-1">※ マルチモーダル対応モデル推奨</p>
            </div>
            <div>
              <label className="block text-gray-300 text-xs mb-1">APIキー</label>
              <input
                type="password"
                value={apiKey}
          ...
          {/* ── 音声 ── */}
          <section className="space-y-2">
            <h3 className="text-gray-400 text-xs font-semibold uppercase tracking-wider">音声エンジン</h3>
            <select
              value={voiceEngine}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-[600px] max-w-[90vw] max-h-[80vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700 shrink-0">
              <span className="text-yellow-400 font-bold text-sm">🐛 チャットログ（デバッグ）</span>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-xs">
              {chatLog.length === 0 ? (
                ...
                chatLog.map((msg, i) => (
                  ...
                  >
                    <span className="font-bold mr-2 opacity-60">[{msg.role}]</span>
